...
# Define files
CSV_FILE="seeds/similarities.csv"
...
# Extract subject_term from similarities.csv (skipping header, first column)
# Normalize: remove brackets [], convert to lowercase, trim whitespace
CSV_TERMS=$(tail -n +2 "$CSV_FILE" | awk -F',' '{print $1}' | tr -d '[]' | tr '[:upper:]' '[:lower:]' | sed 's/^ *//;s/ *$//' | sort -u)
